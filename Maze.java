...
public class Maze {
    ...
    public Maze( String sourceFilename
        ...
        while( sc.hasNextLine() ) {
            ...
            if (line.length() > fileCount) fileCount = line.length();
    ...
    public boolean checkMaybeGo(int dir){
      if(dir == NORTH){
        if (explorerPosition.rank == 0) return false;
        return true;
      }
      if(dir == SOUTH){
        if (explorerPosition.rank == rankCount - 1) return false;
        return true;
      }
      if(dir == EAST){
       if (explorerPosition.file == fileCount - 1) return false;
        return true;
      }
      if(dir == WEST){
        if (explorerPosition.file == 0) return false;
        return true;
      }
     return false;
    }
