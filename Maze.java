...
public class Maze {
	...
	//needs a constructor here i think????? -Steven
	Maze newMaze; //idk about this :(		
	...
	for(int index = 0; index < old.maze.length; index++){
		for(int nextIndex = 0; nextIndex < old.maze[index].length; nextIndex++){
			newMaze[index][nextIndex] = old.maze[index][nextIndex];
		}
	}

	newMaze.rankCount = old.rankCount;
